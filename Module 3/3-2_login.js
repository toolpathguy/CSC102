...
form.onsubmit = function(event)  // set the onsubmit property of the form to a function that will run on form submission
{
    ...
    var uatZip = parseInt(document.getElementById("uatZip").value, 10); // get the value from the uat zip code input
    ...
    if (uatZip !== 85283)  // check if the zip code is not equal to the University of Advancing Technology zip code
    {
    ...
    window.location.href = "/final_project.html"; // redirect to the final project home page using the correct relative path
}